...
class Status:
    ...
    def register_job(self, job_id=None):
        ...
        self.__id_status_dict[job_id] = dict(status='start_job', status_code='1', unix_timestamp=time(), elapsed_time=0)
        return job_id

    def update(self, job_id, status, refresh=True):
        self.__update_message(job_id, status, '1', refresh=refresh)

    def complete(self, job_id, url):
        self.__update_message(job_id, 'completed', '0', url=url, refresh=True)

    def error(self, job_id, error_message):
        self.__update_message(job_id, error_message, '-1', refresh=True)

    def __update_message(self, job_id, status, status_id, url='', refresh=True):
        if job_id not in self.__id_status_dict.keys():
