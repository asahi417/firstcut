...
class CutoffMethods:
    ...
    def get_cutoff_amp(self,
        ...
        if self.__method_type in ['percentile']:
            ...
            raise ValueError('unknown `method_type`: {} not in {}'.format(self.__method_type, VALID_METHOD_TYPES))
    ...
    def __percentile(data, p):
        ...
        logging.debug('cutoff threshold (percentile method)')
        logging.debug(' * percentile: {} with {} percentile'.format(val, p))
        return int(val)
